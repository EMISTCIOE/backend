...
So whenever you try and make any commit, it will automatically run the `linting` and `ruff` on the staged/changed files and if there are any errors, it will not let you commit the code until you fix those.
...
- `feat: email validation for account signup`
