...
from .serializers import (
    ResearchListSerializer, ResearchDetailSerializer, 
    ResearchCreateUpdateSerializer, ResearchCategorySerializer
)
...
class ResearchViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['status', 'research_type', 'department', 'is_featured', 'is_published']
    search_fields = ['title', 'description', 'abstract', 'principal_investigator', 'keywords']
    ...
    def featured(self, request):
        ...
        featured_research = self.get_queryset().filter(is_featured=True, is_published=True)
    ...
    def by_department(self, request):
        ...
        if department_id:
            research = self.get_queryset().filter(department_id=department_id, is_published=True)
    ...
    def by_participant(self, request):
        ...
        if participant_name:
            queryset = queryset.filter(participants__full_name__icontains=participant_name)
